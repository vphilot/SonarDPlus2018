...
function initHome() {
  ...
  home.id = "home";
  home.innerHTML = "<div class=\"logo-wrapper\"><div id=\"logo-primary\"></div><div id=\"logo-secondary\"></div></div><div id=\"header\"><h1>The congress where technology and creative industries converge &#8212</h1><h2>Feb 08 - 10, 2018</h2><h2>Fira Montjuïc, Barcelona</h2></div></div><div id=\"particles-js\"></div>";
...
function initSchedule() {
  ...
  for (let d = 0; d < days.length; d++) {
    ...
    buttons.innerHTML += "<button id=\"day" + (parseInt(d) + 1) + "\"type=\"button\" value=\"" + days[d] + "\"class=\"btn-day\">" + "<strong>" + weekday + "</strong>" + " " + date + "</button>";
  }

  buttons.innerHTML += "<button id=\"btn-filter\" type=\"button\" value=\"true\" class=\"btn-filter\"><i class=\"far fa-dot-circle fa-xs\" data-fa-transform=\"shrink-4\"></i>Filter</button>";
  ...
  btnFilter.addEventListener("click", (e) => {
    ...
    fa.length === 0 ? talksWrapper.innerHTML = "<p class=\"error\">Add talks to your schedule and they'll be displayed here.</p>" :
      (talksWrapper.innerHTML = displayInfo(fa, "talk"),
        talksWrapper.innerHTML += "<button class=\"btn-print\">Print my Schedule</button>",
        initDynamicElements());
  });
  //toggle active
  let btnTalksNav = document.getElementsByClassName("btn-wrapper")[0].getElementsByTagName("button");
...
function initNav() {
  ...
  for (let s = 0; s <= secs.length; s++) {
    s === secs.length ? (
      ...
      initNavEvents()
    ) : (nav.innerHTML += "<a href =#" + secs[s].id + " class=\"nav-link\">" + secs[s].id + "</a>");
  }
}
...
function displayInfo(_filteredArray, _type) {
  ...
  for (let f = 0; f < fa.length; f++) {
    ...
    switch (_type) { //using switch statements to dictate the outcome of my html elements based on the type wanted
      case "talk":
        ...
        output += "<img src=\"" + "../images/" + fa[f].talkImg + "\" alt=\"" + fa[f].title + "\">";
        output += "<h1>" + fa[f].title + "</h1><h2>" + fa[f].speaker + "</h2><h5 class=\"day\">" + String(new Date(fa[f].start)).substr(0, 3) + "</h5><h5 class=\"time\">" + s.substr(16, 5) + " &#8212 " + e.substr(16, 5) + "</h5><p class=\"description\">" + fa[f].description + "</p>";
        break;
      case "speaker":
        output += "<div class=\"speaker\"\ data-hash=\"" + fa[f].hash + "\">";
        output += "<img src=\"" + "../images/" + fa[f].bioImg + "\" alt=\"" + fa[f].speaker + "\">";
        output += "<h1>" + fa[f].speaker + "</h1><p class=\"bio\">" + fa[f].bio + "</p>";
        output += "<h2>Upcoming talks:</h2>";
        output += "<h2>" + fa[f].title.toUpperCase() + "</h2><h5 class=\"day\">" + String(new Date(fa[f].start)).substr(0, 3) + "</h5><h5 class=\"time\">" + s.substr(16, 5) + " &#8212 " + e.substr(16, 5) + "</h5>";
        break;
      default:
        console.log("nothing")
    };
    fa[f].attending ? output += "<button type=\"button\" value=\"attending-true\" class=\"btn-attending true\">" + goingTxt + "</button>" : output += "<button type=\"button\" value=\"attending\" class=\"btn-attending\">" + attendTxt + "</button>";
    output += "</div>";
  }
...
function initTalkDetails() {
  ...
  for (let t = 0; t < talks.length; t++) {
    talks[t].getElementsByTagName("h1")[0].addEventListener("click", (e) => popDetail(e.target.parentElement, "talk"));
  }
}
...
function initSpeakerDetails() {
  ...
  for (let t = 0; t < speakers.length; t++) {
    speakers[t].getElementsByTagName("h1")[0].addEventListener("click", (e) => popDetail(e.target.parentElement, "speaker"));
  }
}
...
function initPrintButton() {
  ...
  btnPrint.addEventListener("click", function(e) {
    ...
    newWindow.document.write("<head> <meta charset=\"utf-8\"> <title>Your Sónar +D 2018 Schedule</title> <style> * {font-family:monospace; color: black;} button{display:none;} img{display:none;} </style> </head><body><h1>Your Sonar 2018 D+ Schedule</h1>");
...
function updateFirebase(_element) {
  ...
  let node = firebase.database().ref("talks").child(parseInt(hash)).child("attending");
  ...
  node.once("value", function(snap) {
    ...
    nodeState === true ? (
        ...
        _element.innerHTML = "Attend", _element.className = "btn-attending") :
      (
        node.set(true),
        nodeInst.attending = true,
        _element.innerHTML = "Going", _element.className = "btn-attending true"
      );
...
function popDetail(_element, _type) {
  ...
  pop.innerHTML += "<button type=\"button\" value=\"back\" class=\"btn-talk-back\"><i class=\"far fa-times-circle\"></i></button>";
  document.body.appendChild(pop);
  pop.getElementsByClassName("btn-talk-back")[0].addEventListener("click", () => {
...
function aFilter(_array, _key, _term) {
  ...
  if (_term == "all") {
    ...
    function filterByTerm(value) {
      if ((typeof value[_key]) === "string") { //IF it is a string, then compare it and return
